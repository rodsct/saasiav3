...
export default function DownloadsGrid({ showAdminControls = false }: DownloadsGridProps) {
  ...
  const getAccessLevelBadge = (level: string) => {
    const styles = {
      ...
      PRO: "bg-orange-900/20 text-orange-400 border-orange-800"
    };
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        {filteredDownloads.map((download) => {
          ...
          return (
            <div
              key={download.id}
              className={`bg-[#3f3f3f] border border-[#4f4f4f] rounded-lg p-6 transition-all hover:border-[#5f5f5f] ${
                  ...
                  {download.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mb-4">
                      {download.tags.slice(0, 4).map((tag, index) => (
                        <span
                          key={index}
                          className="px-2 py-1 text-xs bg-[#ff6b35]/20 text-[#ff6b35] rounded"
                        >
                          {tag}
                        </span>
                      ))}
                      {download.tags.length > 4 && (
                        <span className="px-2 py-1 text-xs bg-[#2f2f2f] text-gray-300 rounded">
                          +{download.tags.length - 4}
                        </span>
                      )}
                    </div>
      ...
      {filteredDownloads.length === 0 && (
        <div className="text-center py-12">
          <div className="w-16 h-16 mx-auto mb-4 bg-[#3f3f3f] rounded-full flex items-center justify-center">
